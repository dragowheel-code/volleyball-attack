...
function formaterMontant(valeur) {
  return Number(valeur ?? 0).toLocaleString("fr-CA", {
    style: "currency",
    currency: "CAD",
  });
}
        ...
        {familleSelectionnee.inscriptions?.length ? (
          familleSelectionnee.inscriptions.map((inscription) => {
            const paiements = Array.isArray(inscription.paiements)
              ...
              : [];
            const montantRecu = paiements
              .filter((paiement) => paiement.statut === "recu")
              .reduce(
                (total, paiement) => total + Number(paiement.montant ?? 0),
                0
              );
            return (
              <div key={inscription.id} className="familles-dossier-carte">
                <strong>
                  {inscription.enfant_prenom} {inscription.enfant_nom}
                </strong>
                <p>
                  {inscription.cours} —{" "}
                ...
                <p>
                  Prix facturé : {formaterMontant(inscription.prix_facture)}
                </p>
                <p>
                  Mode de paiement :{" "}
                  <strong>
                    {inscription.nombre_versements ?? 1} versement
                    {Number(inscription.nombre_versements ?? 1) > 1 ? "s" : ""}
                  </strong>
                </p>
                {paiements.length > 0 ? (
                  <div>
                    ...
                    <p>
                      Total reçu : <strong>{formaterMontant(montantRecu)}</strong>
                    </p>
                  </div>
                ) : (
                  <p>Aucun paiement créé.</p>
                )}
                  ...
                  <div>
                    ...
                    {inscription.date_remboursement && (
                      <p>
                        ...
                        {formaterDate(inscription.date_remboursement)}
                      </p>
                    )}
                    {inscription.note_remboursement && (
                      <p>
                        Note : {inscription.note_remboursement}
                      </p>
                    )}
                  </div>
                )}
