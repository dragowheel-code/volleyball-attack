...
const [courrielParent, setCourrielParent] =
  ...
  async function inviterParent(formulaire) {
    setMessageInvitation("");
    const prenom = formulaire.prenom.trim();
    const nom = formulaire.nom.trim();
    const courriel = formulaire.courriel.trim().toLowerCase();
    const telephone = formulaire.telephone.trim();
    const lien = formulaire.lien.trim();
    if (!prenom || !nom || !courriel || !lien) {
      setMessageInvitation("Veuillez remplir les champs obligatoires.");
      return;
    }
    setInvitationEnCours(true);
    const { data, error } = await supabase.functions.invoke(
      "inviter-deuxieme-parent",
      { body: { prenom, nom, courriel, telephone, lien } }
    );
    setInvitationEnCours(false);
    if (error) {
      console.error("Erreur lors de l'invitation :", error);
      let message = data?.message || "Impossible d'ajouter le deuxième parent.";
      if (error.context) {
        try {
    ...
    if (!data?.success) {
      setMessageInvitation(data?.message || "Impossible d'ajouter le deuxième parent.");
      return;
    }
    await chargerParentsFamille();
    setAfficherAjoutParent(false);
    setMessageInvitation("");
    if (data.type === "invitation_envoyee") {
      alert("L'invitation a été envoyée au deuxième parent.");
    } else if (data.type === "acces_parent_ajoute") {
      alert("L'accès parent a été ajouté au compte existant.");
    } else if (data.type === "parent_existant") {
      alert("Le parent a été rattaché à votre famille.");
    } else {
